...
let productCount = document.querySelector("a.footer-cart-contents>span.count").innerHTML;
let newCart = document.querySelector(".product-count").innerHTML = productCount;
...
// categoryPage button click event
document.getElementById("categoryButton").onclick = ()=>{
    let msg = {
        pageId : "categoryPage",
    };
    message(msg)
...
// mutation observer for updating cart items

// Select the node that will be observed for mutations
const targetNode = document.querySelector("li.cart");
...
function identifyEmptyPage(){
    let url = new URL(window.location.href);
    let postType = url.searchParams.get("post_type");

    if(postType === "product" && document.querySelectorAll("ul.products").length === 0){
        let divForEmptyList = document.createElement("div");
        divForEmptyList.classList.add("emptyProductList");
        document.body.append(divForEmptyList);
        divForEmptyList.innerHTML = emptySearch;

        let spanElement = document.createElement("span");
        spanElement.innerHTML = "Sorry, no result found.";
        divForEmptyList.append(spanElement);

    }

    if(url.pathname === "/cart/" && document.querySelectorAll(".cart-empty").length !== 0){
        let div = document.createElement("div");
        div.classList.add("emptycart");
        document.body.append(div);
        div.innerHTML = emptyCart;

        let spanElement = document.createElement("span");
        spanElement.innerHTML = "Your cart is empty";
