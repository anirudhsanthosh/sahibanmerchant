...
let productCount = document.querySelector(
  "a.footer-cart-contents>span.count"
).innerHTML;
let newCart = (document.querySelector(".product-count").innerHTML =
  productCount);
...
function message(msg) {
  ...
  else window.parent.postMessage(stringifiedMessageObj);
}
...
function identifyEmptyPage() {
  ...
  if (
    postType === "product" &&
    document.querySelectorAll("ul.products").length === 0
  ) {
  ...
  if (
    url.pathname === "/cart/" &&
    document.querySelectorAll(".cart-empty").length !== 0
  ) {
    let div = document.createElement("div");
