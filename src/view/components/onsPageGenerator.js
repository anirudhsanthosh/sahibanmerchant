...
export default function onsPageGenerator(config = {}) {
  if (config.constructor.name !== "Object") {
    throw new Error("config must be an object");
  }
  const id = config.id ?? Math.random().toString(36).substring(2, 15) + ".html";
  const template = ons.createElement(
    `<template id="${id}"><ons-page id="page-${id}" class="product-display-page-from-template"><ons-toolbar modifier="material" class="product-page-toolbar"><div class="left">
